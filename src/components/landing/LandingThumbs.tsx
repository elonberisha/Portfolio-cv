...
const TH = (props: React.HTMLAttributes<HTMLDivElement>) => (
  <div {...props} style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative', ...props.style }} />
)
...
function ThEditorial() {
  return (
    <TH style={{ background: '#f6f1e7', color: '#1a1814', padding: '8px 14px', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ fontSize: 8, letterSpacing: '0.2em', textTransform: 'uppercase' as const, opacity: .6, textAlign: 'center' as const, borderBottom: '1px solid #1a1814', paddingBottom: 4, marginBottom: 6 }}>№ 07 · Spring</div>
      <div style={{ fontFamily: 'Fraunces, serif', fontStyle: 'italic', fontWeight: 900, fontSize: 38, lineHeight: 0.85, textAlign: 'center' as const, marginTop: 14 }}>The<br/>Quiet<br/><span style={{ fontStyle: 'normal' }}>Hours</span></div>
...
function ThBrutalist() {
  return (
    <TH style={{ background: '#efeae2', color: '#0c0c0c', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ padding: '6px 10px', borderBottom: '1px solid #0c0c0c', fontSize: 7, letterSpacing: '0.1em', textTransform: 'uppercase' as const, display: 'flex', justifyContent: 'space-between' }}>
        <span>jordan / index</span><span>toronto · cs</span>
      </div>
      <div style={{ padding: '4px 10px', borderBottom: '1px solid #0c0c0c', overflow: 'hidden', whiteSpace: 'nowrap' as const, fontFamily: 'Archivo Black, sans-serif', fontSize: 13 }}>
        JORDAN PARK · LOOKING FOR · NEW GRAD · AVAILABLE
      </div>
      <div style={{ padding: 12, fontFamily: 'Archivo Black, sans-serif', fontSize: 28, lineHeight: 0.86, letterSpacing: '-0.03em', textTransform: 'uppercase' as const }}>BUILDS<br/>TINY<br/>TOOLS.</div>
      <div style={{ borderTop: '1px solid #0c0c0c', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
        <div style={{ padding: 8, borderRight: '1px solid #0c0c0c', fontSize: 8, fontFamily: 'Archivo Black, sans-serif', textTransform: 'uppercase' as const }}>tinyk8s</div>
        <div style={{ padding: 8, fontSize: 8, fontFamily: 'Archivo Black, sans-serif', textTransform: 'uppercase' as const }}>lex.fish</div>
      </div>
    </TH>
  )
}
    ...
    <TH style={{ background: '#d3d6e0', padding: 6 }}>
      <div style={{ background: '#e6e9f0', border: '1px solid #888', boxShadow: '1px 1px 0 #fff inset, -1px -1px 0 #6d6d8a inset' }}>
        ...
        <div style={{ padding: 8 }}>
          ...
          <div style={{ background: '#000', color: '#00ff80', fontFamily: 'VT323, monospace', fontSize: 10, padding: '2px 6px', marginTop: 6, overflow: 'hidden', whiteSpace: 'nowrap' as const, border: '2px inset #888' }}>★彡 NOW OPEN FOR FREELANCE ★彡</div>
          <div style={{ display: 'flex', gap: 3, marginTop: 6, flexWrap: 'wrap' as const }}>
  ...
  return (
    <TH style={{ background: '#fafaf8', color: '#0d0d0d', fontFamily: 'Inter, sans-serif', padding: 12, fontSize: 8 }}>
      ...
      <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
        {['04', '02', '4.2k', '3'].map((n, i) => (
          <div key={i}><div style={{ fontWeight: 300, fontSize: 18, letterSpacing: '-0.04em', lineHeight: 1 }}>{n}</div><div style={{ fontSize: 6, letterSpacing: '0.1em', textTransform: 'uppercase' as const, opacity: .55, marginTop: 2 }}>{['Projects','Interns','Subs','Lang'][i]}</div></div>
        ))}
      </div>
    </TH>
  )
...
function ThPlayful() {
  return (
    <TH style={{ background: '#fff5d6', color: '#2d1810', padding: 14, fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ position: 'absolute', right: 10, top: 60, background: '#ff7a45', color: '#fff', borderRadius: '50%', width: 50, height: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 7, fontWeight: 700, textAlign: 'center' as const, lineHeight: 1.1, transform: 'rotate(12deg)' }}>★ OPEN<br/>2026 ★</div>
    </TH>
  )
}
...
function Th3D() {
  return (
    <TH style={{ background: '#06070a', color: '#e9ecf3', padding: 14, fontFamily: 'Manrope, sans-serif' }}>
      ...
      <div style={{ position: 'relative', fontSize: 8, opacity: .65, letterSpacing: '0.1em', textTransform: 'uppercase' as const }}>jordan park · cs final year</div>
...
function ThNewspaper() {
  return (
    <TH style={{ background: '#f0e9d5', color: '#1a1610', padding: 10, fontFamily: 'IBM Plex Serif, serif' }}>
      <div style={{ borderBottom: '3px double currentColor', paddingBottom: 4, textAlign: 'center' as const }}>
  ...
  return (
    <TH style={{ background: '#ffffff', color: '#14181f', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ padding: 12, display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 8 }}>
        <div>
          <div style={{ fontSize: 7, color: '#1f4ed8', fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase' as const }}>Available · Sept 2026</div>
    ...
    <TH style={{ background: '#e9e4d8', padding: 8, fontFamily: 'IBM Plex Mono, monospace' }}>
      <div style={{ background: '#fff', color: '#14110b', display: 'grid', gridTemplateColumns: '1fr 60px', boxShadow: '0 4px 12px rgba(0,0,0,.15)' }}>
        ...
        <div style={{ borderLeft: '2px dashed #14110b', padding: 8, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ textAlign: 'center' as const }}><div style={{ fontSize: 5, opacity: .55, letterSpacing: '0.12em' }}>SEAT</div><div style={{ fontWeight: 700, fontSize: 16 }}>24F</div></div>
...
function ThMuseum() {
  return (
    <TH style={{ background: '#ece6db', color: '#1a1714', padding: 14, fontFamily: 'Instrument Serif, serif' }}>
      <div style={{ fontSize: 6, letterSpacing: '0.22em', textTransform: 'uppercase' as const, opacity: .65, fontFamily: 'Inter, sans-serif', textAlign: 'center' as const, borderBottom: '1px solid #1a1714', paddingBottom: 4 }}>FOLIO · GALLERIA · SPRING 2026</div>
      <div style={{ textAlign: 'center' as const, padding: '16px 0 12px', borderBottom: '1px solid #1a1714' }}>
        <div style={{ fontSize: 6, letterSpacing: '0.3em', textTransform: 'uppercase' as const, fontFamily: 'Inter, sans-serif', opacity: .55 }}>Solo Exhibition · Plate № 01—06</div>
    ...
    <TH style={{ background: '#0d1b2a', color: '#e9eef5', padding: 10, fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ position: 'relative', background: 'linear-gradient(155deg, #ffd06f, #ff7a6c)', borderRadius: 8, padding: 2, marginTop: 6 }}>
        <div style={{ background: '#1a0e2c', borderRadius: 6, padding: 10 }}>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 6, letterSpacing: '0.2em', opacity: .65, textTransform: 'uppercase' as const }}>№ 001 / 099 · Legendary</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 5, marginTop: 8, paddingTop: 6, borderTop: '1px solid rgba(255,255,255,.15)' }}>
            {[['14','Repos'],['2.1k','Commits'],['4','Interns'],['3.92','GPA']].map(([n,l],i) => (
              <div key={i}><div style={{ fontFamily: 'Unbounded, sans-serif', fontWeight: 700, fontSize: 11 }}>{n}</div><div style={{ fontSize: 5, opacity: .6, letterSpacing: '0.1em', textTransform: 'uppercase' as const }}>{l}</div></div>
            ))}
          </div>
        </div>
      </div>
...
function ThManifesto() {
  return (
    <TH style={{ background: '#f0ec4a', color: '#100c0c', padding: 10, fontFamily: 'Unbounded, sans-serif' }}>
      <div style={{ fontSize: 6, letterSpacing: '0.18em', textTransform: 'uppercase' as const, fontFamily: 'Inter, sans-serif', borderBottom: '2px solid #100c0c', paddingBottom: 4, display: 'flex', justifyContent: 'space-between' }}>
        <span>Manifesto № 01</span><span>Milan · 2026</span>
      </div>
      <div style={{ fontWeight: 900, fontSize: 32, lineHeight: 0.88, letterSpacing: '-0.04em', textTransform: 'uppercase' as const, marginTop: 10 }}>A<br/><span style={{ WebkitTextStroke: '1px #100c0c', color: 'transparent' } as React.CSSProperties}>PRACTICE</span><br/>IN<br/>PUBLIC.</div>
    </TH>
  )
}
  ...
  return (
    <TH style={{ background: '#f5efde', color: '#221d12', fontFamily: 'EB Garamond, serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 50px', padding: 10, gap: 8 }}>
        <div>
          <div style={{ fontFamily: 'Major Mono Display, monospace', fontSize: 5, letterSpacing: '0.22em', opacity: .65, textTransform: 'uppercase' as const }}>An Atlas of</div>
        ...
        <div style={{ width: 40, height: 40, border: '1px solid currentColor', borderRadius: '50%', position: 'relative', alignSelf: 'flex-end' }}>
  ...
  return (
    <TH style={{ background: '#ddd6c4', padding: 8, fontFamily: 'IBM Plex Mono, monospace' }}>
      <div style={{ background: '#faf6e6', color: '#1a1714', padding: '10px 14px', maxWidth: 200, margin: '0 auto', boxShadow: '0 8px 18px rgba(0,0,0,.15)' }}>
        <div style={{ textAlign: 'center' as const, paddingBottom: 4, borderBottom: '1.5px dashed currentColor' }}>
          <div style={{ fontWeight: 700, fontSize: 12, letterSpacing: '0.06em' }}>FOLIO/MART</div>
          <div style={{ fontSize: 6, opacity: .65, letterSpacing: '0.08em', marginTop: 2, textTransform: 'uppercase' as const }}>★ CV · since 2002 ★</div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: 4, gap: 1 }}>
          {Array.from({length: 20}, (_,i) => <i key={i} style={{ width: i%2 ? 1 : 2, height: 10, background: '#1a1714' }} />)}
        </div>
      </div>
    </TH>
  )
...
function ThLibrary() {
  return (
    <TH style={{ background: '#d8cdb4', color: '#2a2117', padding: 12, fontFamily: 'Special Elite, monospace' }}>
      <div style={{ textAlign: 'center' as const, fontSize: 6, letterSpacing: '0.18em', textTransform: 'uppercase' as const, opacity: .8, borderBottom: '2px solid currentColor', paddingBottom: 4 }}>★ FOLIO PUBLIC LIBRARY ★</div>
  ...
  return (
    <TH style={{ background: '#f0e7d4', color: '#1d1612', fontFamily: 'Bebas Neue, sans-serif' }}>
      <div style={{ padding: 10, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: '#ff5747', mixBlendMode: 'multiply' as const, transform: 'skewY(-4deg) translateY(-30%)', height: '60%' }} />
        <div style={{ position: 'absolute', top: '30%', left: 0, right: 0, bottom: 0, background: '#2456b3', mixBlendMode: 'multiply' as const, transform: 'skewY(3deg) translateY(20%)', height: '60%' }} />
        <div style={{ position: 'relative' }}>
          <div style={{ fontFamily: 'Inter, sans-serif', fontStyle: 'italic', fontSize: 8 }}>Folio.app proudly presents</div>
          <div style={{ fontSize: 38, lineHeight: 0.78, textTransform: 'uppercase' as const, marginTop: 4 }}>Lina<br/><span style={{ display: 'inline-block', transform: 'rotate(-2deg)' }}>Marchetti</span></div>
      ...
      <div style={{ padding: 4, background: '#f7c948', color: '#1d1612', fontSize: 7, letterSpacing: '0.08em', textAlign: 'center' as const }}>★ TONIGHT&apos;S BILL ★ DOORS 7 ★</div>
    </TH>
  )
}
    ...
    <TH style={{ background: '#c6c2bc', padding: 8 }}>
      <div style={{ background: '#fafafa', borderRadius: 6, overflow: 'hidden', border: '0.5px solid rgba(0,0,0,.15)', boxShadow: '0 8px 20px rgba(0,0,0,.2)' }}>
        <div style={{ background: 'linear-gradient(180deg, #ececec, #d8d8d8)', padding: '4px 6px', display: 'flex', gap: 5, alignItems: 'center', borderBottom: '0.5px solid rgba(0,0,0,.15)' }}>
          ...
          <div style={{ flex: 1, textAlign: 'center' as const, fontSize: 8, color: 'rgba(0,0,0,.55)' }}>Jordan — ~/folio</div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr', minHeight: 160 }}>
          <div style={{ background: 'rgba(0,0,0,.025)', borderRight: '0.5px solid rgba(0,0,0,.1)', padding: 6, fontSize: 7 }}>
            <div style={{ opacity: .45, fontSize: 5, letterSpacing: '0.08em', textTransform: 'uppercase' as const, marginBottom: 2 }}>Fav</div>
  ...
  return (
    <TH style={{ background: '#f0eadb', backgroundImage: 'linear-gradient(rgba(80,60,30,.06) 1px, transparent 1px), linear-gradient(90deg, rgba(80,60,30,.06) 1px, transparent 1px)', backgroundSize: '14px 14px', padding: 8, color: '#2a2418', fontFamily: 'EB Garamond, serif' }}>
      <div style={{ border: '1.5px solid currentColor', padding: 10, height: 'calc(100% - 16px)' }}>
        <div style={{ fontFamily: 'Major Mono Display, monospace', fontSize: 5, letterSpacing: '0.22em', opacity: .7, textTransform: 'uppercase' as const, paddingBottom: 4, borderBottom: '1.5px solid currentColor' }}>Herbarium · plate № 014 · Spring 2026</div>
    ...
    <TH style={{ background: '#eceae5', padding: 8 }}>
      <div style={{ background: '#fff', display: 'grid', gridTemplateColumns: '70px 1fr', height: '100%', boxShadow: '0 8px 18px rgba(0,0,0,.15)' }}>
        <aside style={{ background: '#1c1d22', color: '#f3efe7', padding: 8 }}>
          ...
          <div style={{ fontSize: 5, letterSpacing: '0.16em', textTransform: 'uppercase' as const, opacity: .55, marginTop: 8, fontWeight: 600 }}>Skills</div>
          {['Strategy','Finance','SQL','French'].map((s, i) => (
            <div key={i} style={{ marginTop: 3 }}><div style={{ fontSize: 6, marginBottom: 1 }}>{s}</div><div style={{ height: 1.5, background: 'rgba(243,239,231,.18)' }}><i style={{ display: 'block', height: '100%', background: '#f3efe7', width: [95,80,65,80][i]+'%' }} /></div></div>
          ))}
        </aside>
        <main style={{ padding: 8, color: '#1c1d22', fontFamily: 'Inter, sans-serif' }}>
          ...
          <div style={{ fontSize: 5, letterSpacing: '0.2em', textTransform: 'uppercase' as const, opacity: .55, paddingBottom: 2, borderBottom: '1px solid rgba(0,0,0,.1)', fontWeight: 600 }}>Experience</div>
    ...
    <TH style={{ background: '#f4f1ec', padding: 8, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gridAutoRows: 32, gap: 4 }}>
        <div style={{ gridColumn: 'span 4', gridRow: 'span 2', background: '#fff', borderRadius: 8, padding: '6px 8px' }}>
          <div style={{ fontSize: 5, opacity: .5, letterSpacing: '0.14em', textTransform: 'uppercase' as const, fontFamily: 'JetBrains Mono, monospace' }}>About ↗</div>
        ...
        <div style={{ gridColumn: 'span 2', gridRow: 'span 1', background: '#fff', borderRadius: 8, padding: '4px 6px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr' }}>
          {[['06','proj'],['03','jobs'],['03','awd']].map(([n,l],i) => (
            <div key={i} style={{ borderRight: i<2?'1px solid rgba(0,0,0,.08)':'none', padding: '0 3px' }}><div style={{ fontFamily: 'Fraunces, serif', fontWeight: 700, fontSize: 10, letterSpacing: '-0.02em' }}>{n}</div><div style={{ fontSize: 5, opacity: .55, textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>{l}</div></div>
          ))}
        </div>
        <div style={{ gridColumn: 'span 2', gridRow: 'span 1', background: '#ffefe0', borderRadius: 8, padding: '4px 6px' }}>
          <div style={{ fontSize: 5, opacity: .5, fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.14em', textTransform: 'uppercase' as const }}>Now ↗</div>
        ...
        <div style={{ gridColumn: 'span 3', gridRow: 'span 2', background: '#fff', borderRadius: 8, padding: '4px 6px' }}>
          <div style={{ fontSize: 5, opacity: .5, fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.14em', textTransform: 'uppercase' as const }}>Editorial ↗</div>
...
function ThSpare() {
  return (
    <TH style={{ background: '#fbfaf7', color: '#1c1815', padding: '18px 16px', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'JetBrains Mono, monospace', fontSize: 6, letterSpacing: '0.14em', textTransform: 'uppercase' as const, opacity: .55 }}>
      ...
      <div style={{ marginTop: 16, borderTop: '1px solid rgba(28,24,21,.12)', paddingTop: 6, fontSize: 7 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '46px 1fr', padding: '2px 0' }}><span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 6, opacity: .55, letterSpacing: '0.06em', textTransform: 'uppercase' as const }}>Status</span><span>Available · Sep 2026</span></div>
        <div style={{ display: 'grid', gridTemplateColumns: '46px 1fr', padding: '2px 0', borderTop: '1px solid rgba(28,24,21,.12)' }}><span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 6, opacity: .55, letterSpacing: '0.06em', textTransform: 'uppercase' as const }}>Focus</span><span>Print · identity · type</span></div>
      </div>
    </TH>
  )
}
  ...
  return (
    <TH style={{ background: '#f7f5f0', color: '#1a1714', padding: '10px 12px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ marginTop: 8 }}>
        <div style={{ fontSize: 5, opacity: .55, letterSpacing: '0.14em', fontFamily: 'JetBrains Mono, monospace', textTransform: 'uppercase' as const }}>Featured · May 8</div>
...
/** Slug → Thumb component, so DB-driven sections can resolve previews by slug. */
export const THUMB_MAP: Record<string, () => React.ReactElement> = Object.fromEntries(
  TEMPLATE_THUMBS.map((t) => [t.id, t.Thumb]),
)
